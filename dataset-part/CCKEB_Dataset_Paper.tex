...
\memeic{} + GC (Exp3)       & 0.482 [$+$2.27\,pp] & 0.352 & 0.038 & 0.600 \\
\memeic{} + CBD (Exp4)$^\dagger$ & \textbf{0.483} [$+$2.40\,pp] & \textbf{0.363} & 0.047 & 0.600 \\
\textbf{\memeic{} No-connector} & 0.482 [$+$2.32\,pp] & 0.353 & 0.046 & 0.600 \\
\bottomrule
...
\textbf{Observations.}
Heuristic gating (AMG) does not improve performance, suggesting that simple fusion adjustments are insufficient for resolving compositional conflicts. Soft retrieval (STK) yields only marginal gains, indicating that improving retrieval alone does not address the underlying challenges. In contrast, confidence-based deferral (CBD) achieves the highest overall Edit Accuracy (0.483), while the Gated Connector (GC) yields only marginal gains (0.482) comparable to the no-connector variant, indicating that learned fusion does not provide a reliable advantage under adversarial conditions. These results highlight the difficulty of improving multimodal compositional editing through fusion repair alone.
...
+ STK (Exp2)        & \textbf{0.794} & 0.009 & 0.398 & \textbf{0.596} & 0.507 \\
+ GC (Exp3)         & 0.744 & 0.084 & 0.426 & 0.528 & 0.621 \\
...
Figure~\ref{fig:qualitative} provides a side-by-side comparison of the \memeic{} baseline and the \memeic{}+GC variant on five representative Adversarial-2k samples---one per failure category.
